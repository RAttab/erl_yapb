...
void parse_type(ErlNifEnv *env, struct pb_field_cache *field, ERL_NIF_TERM type)
{
    ...
    if (type == state->atom_int32) { field->type = pb_32_int; }
    else if (type == state->atom_uint32) { field->type = pb_32_uint; }
    else if (type == state->atom_sint32) { field->type = pb_32_sint; }
    else if (type == state->atom_int64) { field->type = pb_64_int; }
    else if (type == state->atom_uint64) { field->type = pb_64_uint; }
    else if (type == state->atom_sint64) { field->type = pb_64_sint; }
